export default function AllowedTopicsEditor({ topics, onChange }) {
  const update = (value) => onChange(value.split("\n").map((t) => t.trim()).filter(Boolean));

  return (
    <div className="space-y-2">
      <label className="text-sm font-medium text-slate-700">Allowed Topics</label>
      <textarea
        rows={4}
